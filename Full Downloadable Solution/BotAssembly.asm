...
Init:
	...
	LOAD   GT        ; An indication that we are Alive
	OUT    SSEG2
...
Main: 
	...
	CALL StartLog	; Start Log to Server
...
StartNew:
	; Set up paramaters for ARCTAN
	IN	   LPOS
	STORE  PosBeforeTurn
	ILOAD  DestPtrX ; Paramaters are passed as the difference between the bot units position and t5he odometry
	STORE  Temp
	IN	   XPOS
	STORE  Temp2
	LOAD   Temp
	...
	CALL   Atan2 ; Call Arctan and display ion sseg2 
	OUT   SSEG2
	STORE Temp
	STORE	CurrentAnglePath	; The angle we want to stay pointed at as we travel to point -> For auto correction
	
	ADDI -5; If condition for fail case 0^*	TestingA
	JNEG MoveForward
...
TurnCW:
	...
	OUT LVELCMD
	LOAD RTest	;TESTING
	OUT RVELCMD			;turn clockwise (right)
	IN THETA
	OUT SSEG1
	JZERO TurnCW
	SUB  TEMP
...
StopTurn:
	IN   LPOS			;position after turn - posbefore turn = turn amt
	SUB  PosBeforeTurn
	STORE Temp
	ADD  TurnAmount
	STORE TurnAmount
...
; 	IN     XIO         ; XIO contains KEYs
; 	AND    Mask2       ; KEY3 mask (KEY0 is reset and can't be read)
; 	JPOS   WaitForUser4 ; not ready (KEYs are active-low, hence JPOS)
; 	LOAD   Zero
; 	OUT    XLEDS 
		
;---------------------------------------------------------------- Naveens Insert ^^^
	
	; FUNCTION FOR GETTING TO BOT TO MOVE FOWARD AND SLOW DOWN PROPORTIONALLY
MoveForward:

	IN		LPOS
	SUB		TurnAmount
	STORE 	DistBeforeMove
	
					
		
	; Load variables for distqance function
	ILOAD DestPtrX 	; Load the 1st X data point
	STORE Temp
	IN XPOS
	SUB Temp
	STORE L2X	   	; Store in X
	ILOAD DestPtrY	; Store the Y value in L2Y
	STORE Temp
	IN YPOS
	SUB Temp
	STORE L2Y
	...
	CALL L2Estimate 	;Call the distance function
	STORE DisToTravel	; Store distance in distance 1

			
; 	OUT 	SSEG1		; DIsplay Distance to be travel
...
FastSpeed:	
	...
	STORE SpeedL
	LOAD DisToTravel
	ADDI -276 			;Decelleration threshold
	STORE StartDecelDist
	STORE TEMP1
	JUMP MoveForward1	
				
MoveForward1:
	LOAD SpeedL;			;Load the speed and then get the wheels to turn at that speed
	OUT	 LVELCMD
	LOAD SpeedR
	OUT	 RVELCMD
	IN 	 LPOS	
 	SUB  TurnAmount  	;Load in the Right Wheel Position
	SUB  DistBeforeMove
	STORE  DisTraveled
; 	OUT		SSEG2		;
	...
	SUB CurrentAnglePath

	CALL Abs
	SUB Five
	JZero	NoAdjust1
	JNeg	NoAdjust1	
	CALL 	Adjust		; --Still need to implment subroutine
	;----END AUTO CORRECTION----------------
...
NoAdjust1:
	...
	LOAD	StartDecelDist
	SUB		DisTraveled
	JPOS    MoveForward1 ;Once zero stop
	LOAD	ZERO
	ADDI	276 
	STORE   AmtToDecel
	Load 	Temp1
...
MidSpeed:
	...
	STORE SpeedL	 	
	LOAD DisToTravel
	ADDI -120 			;Decelleration threshold
	STORE StartDecelDist
	Jump MoveForward2
	STORE TEMP2
...
NoAdjust2:
	...
	LOAD	StartDecelDist
	SUB		DisTraveled
	JPOS    MoveForward2 ; Once zero stop
	LOAD	ZERO
	ADDI	120 
	STORE   AmtToDecel
...
; 	IN     XIO         ; XIO contains KEYs
; 	AND    Mask2       ; KEY3 mask (KEY0 is reset and can't be read)
; 	JPOS   WaitForUser1 ; not ready (KEYs are active-low, hence JPOS)
; 	LOAD   Zero
; 	OUT    XLEDS 

	LOAD Count ; Load the count and do decrement test	
	JZERO Die
	ADDI -1
	STORE COUNT
...
; falls through from above.
	...
	CALL 	Wait2		;THIS IS TO PICK UP LAST POINT IN FEED
...
; Adjust forward movement to stay on correct angle  STILL NEEDS TO BE FINISHED
Adjust:
Adjust2:
; 	LOAD CurrentAnglePath
; 	JPOS Normal
;     JNEG Normal
; 	IN THETA
; 	SUB 350
...
; 	SUB CurrentAnglePath
	...
	IN THETA
	SUB Deg180
	JPOS ThetaGreater180	; if Theta greater than 180
	
	IN THETA
	ADD Deg180
	SUB CurrentAnglePath
	JNEG VeeringLeft		; Above range Theta + 180 -> Turn Right
	IN THETA
	SUB CurrentAnglePath	
	JPOS VeeringLeft		; Under range Theta	-> Turn Right
	JUMP VeeringRight		; Else Turn left
	
ThetaGreater180:
	IN THETA
	SUB Deg180
	SUB CurrentAnglePath	
	JPOS VeeringRight		; Under range Theta - 180 -> Turn Left
	IN THETA
	SUB CurrentAnglePath	
	JNEG VeeringRight		; Above range Theta	-> Turn Left
	JUMP VeeringLeft		; Else Turn Right
	
;--Check if veering to right -> Slow left wheel
VeeringRight:
	LOAD SpeedL
	ADDI -200
	OUT	 LVELCMD
...
;--Check if veering to left -> Slow right wheel
VeeringLeft:
	LOAD SpeedR
	ADDI -200
	OUT	 RVELCMD
...
; DECELERATE
Decell: 		; CALL Wait2 
			LOAD AmtToDecel	; Loads in the amount to decell from function move foward 1 or mover foward 2
			OUT sseg1
	
	DecellForward:
			LOAD fslow		;Load the speed and then get the wheels to turn at that speed
 			OUT	 LVELCMD
 			OUT	 RVELCMD
 			IN 	 LPOS	 	
  			SUB	 TurnAmount
 			SUB	 DistBeforeMove					;Load in the Right Wheel Position
 			...
 			OUT		SSEG2
			SUB		DisTraveled
			JPOS    DecellForward ; Once zero stop
...
Temp2: DW 0
Temp3: DW 0
CurrentAnglePath:	DW 0
CDX: DW 0      ; current desired X
CDY: DW 0      ; current desired Y
CDT: DW 0      ; current desired angle
CX:  DW 0      ; sampled X
PosBeforeTurn:  DW 0      ; Lposition before starting turn sequence
TurnAmount:  DW 0      ; Accumlative amount the left wheel is turning
SpeedR:		DW 0 ; current speed
SpeedL:		DW 0
;***************************************************************
;* Constants
...
;---- OUT FILE 
	...
	ALIVE:      	DW &H0A1E   ; Example of a "local variable
	GT:     		DW &H1111   ; Example of a "local variable"
	GT8:	        DW &H0BAD	; Displays BAD
	Print:          DW &H2222   ; Example of a "local variable"
	ACCESSED:       DW &HFFFF   ; Example of a "local variable"
	Dcellerating: 	DW &HDEC1	; Displays Decellerating 
	Midaccessed:	DW &H03ED	; Displays Mid
	ENDED:			DW &HD0E5
	Fast:			DW &HFA57
	Velocity: DW 0
	DisTraveled: DW 0	; Deceleration 
	DisMinus1: DW 0
	distbeforemove:	DW 0
	distotravel:  DW 0
	startdeceldist:  DW 0
	...
	Count: DW 11 ; 13 is the # of data pts. +1
	; Initilize the pointer
	DestPtrX: DW 0 ; Stores a pointer for the data table for position X
	; Initilize the pointer
	DestPtrY: DW 0 ; Stores a pointer for the data table for position Y
	; Initilize the pointer
	PositionPtr: DW 0 ; Stores a pointer for the data table for position Y
